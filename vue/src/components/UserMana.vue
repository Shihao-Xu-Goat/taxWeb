<template>
  <div v-loading="loading">
    <div style="margin-top: 10px;display: flex;justify-content: center">
      <el-input
        placeholder="Displaying, use username to search..."
...
<!--          <div><img :src="user.userface" :alt="user.nickname" style="width: 70px;height: 70px"></div>-->
          ...
          <div
            ...
            <el-switch
              ...
              inactive-text="disable" style="font-size: 12px">
            </el-switch>
          </div>
          <div style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px">
            <span>Role:</span>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      deleteUser(id){
        var _this = this;
        this.$confirm('Delete this user, confirm?', 'Warning', {
          ...
          deleteRequest("/admin/user/" + id).then(resp=> {
            if (resp.status == 200 && resp.data.status == 'success') {
              _this.$message({type: 'success', message: 'Successfully to delete'})
              _this.loadUserList();
              return;
            }
            _this.loading = false;
          ...
          _this.$message({
            type: 'info',
            message: 'Delete action is canceled'
          });
        });
      },
      enabledChange(enabled, id, index){
        ...
        putRequest("/admin/user/enabled", {enabled: enabled, uid: id}).then(resp=> {
          if (resp.status != 200) {
            _this.$message({type: 'error', message: 'Failed to update!'})
            _this.loadOneUserById(id, index);
            return;
          }
          _this.cardloading.splice(index, 1, false)
          _this.$message({type: 'success', message: 'Successfully updated!'})
        }, resp=> {
          _this.$message({type: 'error', message: 'Failed to update!'})
          _this.loadOneUserById(id, index);
        });
